document.addEventListener('DOMContentLoaded', () => { initSkins().then(()=>initSite().catch(console.error)); });
...
async function initSkins(){
  try{
    ...
    const skinToUrl = (n)=> `assets/skins/blocks/${n}.png`;
    const setSkin = (id, skin, light=false, titleSkinName=(cfg.titles||null))=>{
      const sec = document.getElementById(id); if(!sec) return;
      sec.dataset.skin = skin; if(light) sec.classList.add('light'); else sec.classList.remove('light');
      sec.style.setProperty('--skin-url', `url('${skinToUrl(skin)}')`);
      if(titleSkinName){ sec.style.setProperty('--title-skin-url', `url('${skinToUrl(titleSkinName)}')`); }
    };
    setSkin('intro', cfg.intro||'brush-dark', false);
    setSkin('galerie', cfg.galerie||'brush-dark', false);
    setSkin('supports', cfg.supports||'brush-light', true);
    setSkin('preferences', cfg.preferences||'brush-dark', false);
    setSkin('contact', cfg.contact||'brush-light', true);
  }catch(e){}
}
/* Galerie */
async function buildSlider(){
  ...
  const slides=[...slider.querySelectorAll('.slide')]; let current=0; const show=idx=>slides.forEach((s,j)=>s.classList.toggle('active', j===idx));
...
function probeGallery(prefix, exts, max){
  const tries=[]; for(let i=1;i<=max;i++){ const nn=String(i).padStart(2,'0'); for(const ext of exts){ const url=`${prefix}${nn}${ext}`; tries.push(testImage(url).then(ok=>ok?url:null)); } }
  return Promise.all(tries).then(a=>a.filter(Boolean));
}
function testImage(url){ return new Promise(res=>{ const i=new Image(); let done=false; const f=o=>{if(!done){done=true;res(o);}}; i.onload=()=>f(true); i.onerror=()=>f(false); i.src=url+(url.includes('?')?'&':'?')+'v='+Date.now();}); }
function preloadImages(urls){ return Promise.all(urls.map(u=>new Promise(r=>{ const i=new Image(); i.onload=i.onerror=()=>r(); i.src=u;}))); }
/* Bandeaux */
async function buildAccordion(){
  ...
  for(const it of cfg){ if(!it.enabled) continue; const code=it.code, display=it.name||code, folder=`assets/bandeaux/${code}`, image=it.image?`${folder}/${it.image}`:`${folder}/image.png`;
  ...
  items.forEach(it=>{ const header=it.querySelector('.accordion-header'); const cb=it.querySelector('.material-checkbox');
